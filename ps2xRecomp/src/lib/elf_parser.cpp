...
namespace ps2recomp
{
    ...
    std::vector<Function> ElfParser::extractFunctions() const
    {
        ...
        // Named debug functions and every loaded map function have authoritative
        // explicit bounds. Auto-generated names from the JAL fallback do not.
        for (const auto &extra : m_extraFunctions)
        {
            const bool fromGhidraMap =
                m_hasLoadedGhidraMap && m_ghidraMapStarts.contains(extra.start);
            if (extra.start == 0 || extra.end <= extra.start || extra.name.empty() ||
                (IsAutoGeneratedName(extra.name) && !fromGhidraMap))
            {
                continue;
            }
    ...
    bool ElfParser::loadGhidraFunctionMap(const std::string &mapPath)
    {
        ...
        if (count > 0)
        {
            ...
            // A supplied map replaces the broad JAL-only fallback. In particular,
            // do not let a fallback sub_* range win merely because it is larger
            // than an authoritative Ghidra FUN_* range at the same start.
            ...
            m_extraFunctions.insert(
                m_extraFunctions.end(), mapFunctions.begin(), mapFunctions.end());
